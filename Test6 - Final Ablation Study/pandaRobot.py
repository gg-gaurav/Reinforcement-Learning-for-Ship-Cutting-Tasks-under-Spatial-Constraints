...
class pandaRobot(PyBulletRobot):
    ...
    The Observation Space is a `ndarray` with shape `(18,)` representing the end effector positions and the
    ...
    | 13  | rq_x        | x of the relative quaternion btwn current and target orientation  |
    | 14  | rq_y        | y of the relative quaternion btwn current and target orientation  |
    | 15  | rq_z        | z of the relative quaternion btwn current and target orientation  |
    | 16  | rq_w        | w of the relative quaternion btwn current and target orientation  |
    | 17  | ee_progress | ratio of the number of points reached                             |
    ...
    The Action Space is an ndarray and can take 2 forms. 
        ...
        | 3   | roll          | roll to reach target orientation                |
        | 4   | pitch         | pitch to reach target orientation               |
        | 5   | yaw           | yaw to reach target orientation                 |
    ...
    def __init__(self, 
        ...
        self.n_actions = 6 if self.control_type == "joints" else 7  # Action Space: ----------------------------------------------
                                                                    #   Joint Angle Control ==> 6
                                                                    #   End Effector Control ==> 7  
        action_space = spaces.Box(-1.0, 1.0, shape= (self.n_actions,), dtype= np.float32)

        self.body_name = "panda"                                      # choose the name you want
        self.file_name = "franka_panda/panda.urdf"                    # the path of the URDF file 
        ...
        self.ee_link = 11
        self.timestep_scale = 0.05                                  # percentage of the delta we want to use in the current timestep
    ...
    def set_action(self, action: np.ndarray) -> None:
        ...
        Parameters:
        action (ndarray) --- joint angle control - (7,) | EE control - (6,)            
        """
    ...
    def JointAngleControl_To_JointAngles(self, joint_disp) -> np.ndarray:
        ...
        """
        joint_disp = joint_disp[0:7] * self.timestep_scale          # limiting the amount of displacement in a timestep
    ...
    def EEControl_To_JointAngles(self, ee_delta: np.ndarray) -> np.ndarray:
        ...
        """
        # Position:
        ee_disp = ee_delta[:3] * self.timestep_scale                 # limiting the amount of displacement in a timestep                      
        target_ee_position = self.get_ee_position()  + ee_disp
        # Clip the height target. For some reason, it has a great impact on learning
        target_ee_position[2] = np.max((0, target_ee_position[2]))
        
        # Orientation: 
        ee_orient_disp = ee_delta[3:] * self.timestep_scale
        ee_orient_disp = np.clip(ee_orient_disp, -np.pi, np.pi)
        # target_ee_orientation = self.get_ee_orientation() + ee_orient_disp  # Get final orientation quaternion
        current_q = R.from_quat(self.get_ee_orientation())
        delta_q = R.from_rotvec(ee_orient_disp)   
        target_ee_orientation = (delta_q * current_q).as_quat()

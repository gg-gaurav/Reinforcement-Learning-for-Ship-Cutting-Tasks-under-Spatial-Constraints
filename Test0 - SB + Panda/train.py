...
from pandaReachTaskEnv import pandaReachTaskEnv
from panda_gym.pybullet import PyBullet
from customCallback import customCallback
from DecayingOUNoise import DecayingOUNoise
...
buffer_size = int(1e6)
"""
replayBufferClass = HerReplayBuffer
rb_kwargs= {'goal_selection_strategy': 'future',
            'n_sampled_goal': 4}
learning_starts = max_steps * 10
"""

n_actions = env.action_space.shape[-1]
# ou_noise = OrnsteinUhlenbeckActionNoise(mean= np.zeros(n_actions), sigma= 0.5* np.ones(n_actions))
# ou_noise = DecayingOUNoise(mean= np.zeros(n_actions), sigma= 0.5,
...
TIMESTEPS = 10000                                                   # Partial Training: total steps = 10000 * 20 = 200000
